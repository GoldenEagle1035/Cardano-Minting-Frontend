...
export default function collectionReducer(state = collectionobj, { type, payload }) {
  ...
  switch (type) {
    ...
    case types.SET_SHOWASSET:
      console.log("[dream log] reducer SET_SHOWASSET", payload);
      return {
        ...state,
        show_asset: payload
      };

    case types.ADD_MINTING_COLLECTION:
      console.log("[dream log] reducer ADD_MINTING_COLLECTION", payload);
      return {
        ...state,
        minting_collections: [...state.minting_collections, payload.data.collection]
      };

    case types.GET_MINTING_COLLECTIONS:
      console.log("[dream log] reducer GET_MINTING_COLLECTIONS", payload);
      return {
        ...state,
        minting_collections: payload.data.collections
      };

    case types.PUT_MINTED_COLLECTION:
      console.log("[dream log] reducer PUT_MINTED_COLLECTION", payload);
      let temp_minting_collections = [...state.minting_collections];
      for(let i = 0; i < temp_minting_collections.length; i++)
        if(temp_minting_collections[i].ID == payload.data.collection.ID)
          temp_minting_collections[i] = payload.data.collection;
      console.log("[dream log] reducer PUT_MINTED_COLLECTION", temp_minting_collections);
      return {
        ...state,
        minting_collections: temp_minting_collections
      };

    case types.SET_MINTING_STATE:
      console.log("[dream log] reducer SET_MINTING_STATE", payload);
      return {
        ...state,
        minting_state: payload
      }
